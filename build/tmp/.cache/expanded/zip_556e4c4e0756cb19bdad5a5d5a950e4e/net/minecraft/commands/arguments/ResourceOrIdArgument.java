...
public class ResourceOrIdArgument<T> implements ArgumentType<Holder<T>> {
    ...
    public Holder<T> parse(StringReader pReader) throws CommandSyntaxException {
        return this.parse(pReader, VALUE_PARSER);
    }

    @Nullable
    private <O> Holder<T> parse(StringReader pReader, TagParser<O> pParser) throws CommandSyntaxException {
        RegistryOps<O> registryops = this.registryLookup.createSerializationContext(pParser.getOps());
        Dynamic<?> dynamic = parseInlineOrId(registryops, pParser, pReader);
        return !this.hasRegistry ? null : this.codec.parse(dynamic).getOrThrow(p_334690_ -> ERROR_FAILED_TO_PARSE.createWithContext(pReader, p_334690_));
    }

    @VisibleForTesting
    static <T> Dynamic<T> parseInlineOrId(DynamicOps<T> pOps, TagParser<T> pTagParser, StringReader pReader) throws CommandSyntaxException {
        int i = pReader.getCursor();
        T t = pTagParser.parseAsArgument(pReader);
        if (hasConsumedWholeArg(pReader)) {
            return new Dynamic<>(pOps, t);
        } else {
            pReader.setCursor(i);
            ResourceLocation resourcelocation = ResourceLocation.read(pReader);
            if (hasConsumedWholeArg(pReader)) {
                return new Dynamic<>(pOps, pOps.createString(resourcelocation.toString()));
            } else {
                pReader.setCursor(i);
