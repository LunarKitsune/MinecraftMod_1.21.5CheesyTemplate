...
public class ChunkMap extends ChunkStorage implements ChunkHolder.PlayerProvider, GeneratingChunkMap {
    ...
    public ChunkMap(
        ...
        TicketStorage pTicketStorage,
        int pServerViewDistance,
        boolean pSync
    ) {
        super(
        ...
        this.distanceManager = new ChunkMap.DistanceManager(pTicketStorage, pDispatcher, pMainThreadExecutor);
        ...
        this.setServerViewDistance(pServerViewDistance);
    ...
    private CompletableFuture<ChunkResult<List<ChunkAccess>>> getChunkRangeFuture(ChunkHolder pChunkHolder, int pRange, IntFunction<ChunkStatus> pStatusGetter) {
        if (pRange == 0) {
            ...
            return pChunkHolder.scheduleChunkGenerationTask(chunkstatus1, this).thenApply(p_326378_ -> p_326378_.map(List::of));
        } else {
    ...
    public CompletableFuture<ChunkResult<LevelChunk>> prepareEntityTickingChunk(ChunkHolder pChunk) {
        return this.getChunkRangeFuture(pChunk, 2, p_326375_ -> ChunkStatus.FULL)
    ...
    private void scheduleUnload(long pChunkPos, ChunkHolder pChunkHolder) {
        ...
        completablefuture.thenRunAsync(() -> {
            ...
            if (p_358669_ != null) {
                LOGGER.error("Failed to save chunk {}", pChunkHolder.getPos(), p_358669_);
            }
        });
    }
    ...
    private CompletableFuture<ChunkAccess> scheduleChunkLoad(ChunkPos pChunkPos) {
        CompletableFuture<Optional<SerializableChunkData>> completablefuture = this.readChunk(pChunkPos)
            .thenApplyAsync(p_358681_ -> p_358681_.map(p_390136_ -> {
                SerializableChunkData serializablechunkdata = SerializableChunkData.parse(this.level, this.level.registryAccess(), p_390136_);
        ...
        return completablefuture.<Object, Optional<SerializableChunkData>>thenCombine(
            ...
            .thenApplyAsync(p_358671_ -> {
                ...
                if (p_358671_.isPresent()) {
                    ChunkAccess chunkaccess = p_358671_.get().read(this.level, this.poiManager, this.storageInfo(), pChunkPos);
            ...
            .exceptionallyAsync(p_326410_ -> this.handleChunkLoadFailure(p_326410_, pChunkPos), this.mainThreadExecutor);
    }
    ...
    public CompletableFuture<ChunkResult<LevelChunk>> prepareTickingChunk(ChunkHolder pHolder) {
        CompletableFuture<ChunkResult<List<ChunkAccess>>> completablefuture = this.getChunkRangeFuture(pHolder, 1, p_326384_ -> ChunkStatus.FULL);
        CompletableFuture<ChunkResult<LevelChunk>> completablefuture1 = completablefuture.thenApplyAsync(p_358666_ -> p_358666_.map(p_358689_ -> {
            ...
            if (completablefuture2.isDone()) {
                ...
                completablefuture2.thenAcceptAsync(p_296572_ -> this.onChunkReadyToSend(pHolder, levelchunk), this.mainThreadExecutor);
            }
    ...
    private boolean save(ChunkAccess pChunk) {
        ...
        if (!pChunk.tryMarkSaved()) {
            ...
            try {
                ...
                net.minecraftforge.event.ForgeEventFactory.onChunkDataSave(pChunk, pChunk.getWorldForge() != null ? pChunk.getWorldForge() : this.level, serializablechunkdata);
    ...
    private CompletableFuture<Optional<CompoundTag>> readChunk(ChunkPos pPos) {
        return this.read(pPos).thenApplyAsync(p_214907_ -> p_214907_.map(this::upgradeChunkTag), Util.backgroundExecutor().forName("upgradeChunk"));
    }
    ...
    void collectSpawningChunks(List<LevelChunk> pOutput) {
    ...
    boolean anyPlayerCloseEnoughForSpawning(ChunkPos pChunkPos) {
        TriState tristate = this.distanceManager.hasPlayersNearby(pChunkPos.toLong());
        return tristate == TriState.DEFAULT ? this.anyPlayerCloseEnoughForSpawningInternal(pChunkPos) : tristate.toBoolean(true);
    }
    ...
    private boolean playerIsCloseEnoughForSpawning(ServerPlayer pPlayer, ChunkPos pChunkPos) {
        if (pPlayer.isSpectator()) {
            ...
            double d0 = euclideanDistanceSquared(pChunkPos, pPlayer.position());
            return d0 < 16384.0;
        }
    }

    private static double euclideanDistanceSquared(ChunkPos pChunkPos, Vec3 pPos) {
        double d0 = SectionPos.sectionToBlockCoord(pChunkPos.x, 8);
        double d1 = SectionPos.sectionToBlockCoord(pChunkPos.z, 8);
        double d2 = d0 - pPos.x;
        double d3 = d1 - pPos.z;
        return d2 * d2 + d3 * d3;
    }
    ...
    private void updateChunkTracking(ServerPlayer pPlayer) {
        ...
        if (!(
            pPlayer.getChunkTrackingView() instanceof ChunkTrackingView.Positioned chunktrackingview$positioned
    ...
    class DistanceManager extends net.minecraft.server.level.DistanceManager {
        protected DistanceManager(final TicketStorage pTicketStorage, final Executor pDispatcher, final Executor pMainThreadExecutor) {
            super(pTicketStorage, pDispatcher, pMainThreadExecutor);
        }
    ...
    class TrackedEntity {
        ...
        public TrackedEntity(final Entity pEntity, final int pRange, final int pUpdateInterval, final boolean pTrackDelta) {
            this.serverEntity = new ServerEntity(ChunkMap.this.level, pEntity, pUpdateInterval, pTrackDelta, this::broadcast, this::broadcastIgnorePlayers);
            this.entity = pEntity;
            this.range = pRange;
        ...
        public void broadcastIgnorePlayers(Packet<?> pPacket, List<UUID> pIgnoredPlayers) {
            for (ServerPlayerConnection serverplayerconnection : this.seenBy) {
                if (!pIgnoredPlayers.contains(serverplayerconnection.getPlayer().getUUID())) {
                    serverplayerconnection.send(pPacket);
                }
            }
        }
