                            ...
                            (p_138084_, p_138085_) -> {
                                ...
                                return SharedSuggestionProvider.suggest(
                                    playerlist.getPlayers()
                                        .stream()
                                        .filter(p_390066_ -> !playerlist.isOp(p_390066_.getGameProfile()))
                                        .map(p_390064_ -> p_390064_.getGameProfile().getName()),
                                    p_138085_
                                );
                            }
                        )
    ...
    private static int opPlayers(CommandSourceStack pSource, Collection<GameProfile> pGameProfiles) throws CommandSyntaxException {
        ...
        for (GameProfile gameprofile : pGameProfiles) {
            if (!playerlist.isOp(gameprofile)) {
                ...
                i++;
                pSource.sendSuccess(() -> Component.translatable("commands.op.success", gameprofile.getName()), true);
            }
        }
