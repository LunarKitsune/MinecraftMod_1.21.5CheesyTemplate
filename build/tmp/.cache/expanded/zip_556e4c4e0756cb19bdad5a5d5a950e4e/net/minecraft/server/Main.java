...
public class Main {
        ...
        try {
            ...
            try {
                ...
                worldstem = Util.<WorldStem>blockUntilDone(
                        p_358505_ -> WorldLoader.load(
                            worldloader$initconfig,
                            p_358511_ -> {
                                Registry<LevelStem> registry = p_358511_.datapackDimensions().lookupOrThrow(Registries.LEVEL_STEM);
                                if (dynamic1 != null) {
                                    LevelDataAndDimensions leveldataanddimensions = LevelStorageSource.getLevelDataAndDimensions(
                                        dynamic1, p_358511_.dataConfiguration(), registry, p_358511_.datapackWorldgen()
                                    );
                                    ...
                                    if (optionset.has(optionspec2)) {
                                        ...
                                        worlddimensions = WorldPresets.createNormalWorldDimensions(p_358511_.datapackWorldgen());
                                    } else {
                                        ...
                                        levelsettings = new LevelSettings(
                                            ...
                                            false,
                                            new GameRules(p_358511_.dataConfiguration().enabledFeatures()),
                                            p_358511_.dataConfiguration()
                                        );
                                    ...
                                    var dynamicops = net.minecraft.resources.RegistryOps.create(net.minecraft.nbt.NbtOps.INSTANCE, p_358511_.datapackWorldgen());
            ...
            final MinecraftServer dedicatedserver = MinecraftServer.spin(
                p_296433_ -> {
                    ...
                    // TODO: [Forge][GameTest] URGENT !!! FIX GAME TESTS !!!
                    /*
                    if (gametestEnabled) {
    ...
    private static void forceUpgrade(
        ...
        boolean pEraseCache,
        BooleanSupplier pRunning,
        RegistryAccess pRegistryAccess,
        boolean pRecreateRegionFiles
    ) {
        LOGGER.info("Forcing world upgrade!");

        try (WorldUpgrader worldupgrader = new WorldUpgrader(pLevelStorage, pDataFixer, pWorldData, pRegistryAccess, pEraseCache, pRecreateRegionFiles)) {
