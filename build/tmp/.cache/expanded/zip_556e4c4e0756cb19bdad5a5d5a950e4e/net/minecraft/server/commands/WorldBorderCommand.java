...
public class WorldBorderCommand {
                        ...
                        .then(
                            Commands.argument("distance", DoubleArgumentType.doubleArg(-5.999997E7F, 5.999997E7F))
                                .executes(
                                    p_390131_ -> setSize(
                                        p_390131_.getSource(),
                                        p_390131_.getSource().getLevel().getWorldBorder().getSize() + DoubleArgumentType.getDouble(p_390131_, "distance"),
                                        0L
                                    )
                                )
                                .then(
                                    Commands.argument("time", IntegerArgumentType.integer(0))
                                        .executes(
                                            p_390130_ -> setSize(
                                                p_390130_.getSource(),
                                                p_390130_.getSource().getLevel().getWorldBorder().getSize() + DoubleArgumentType.getDouble(p_390130_, "distance"),
                                                p_390130_.getSource().getLevel().getWorldBorder().getLerpRemainingTime()
                                                    + IntegerArgumentType.getInteger(p_390130_, "time") * 1000L
                                            )
                                        )
                                )
                        )
    ...
    private static int setDamageBuffer(CommandSourceStack pSource, float pDistance) throws CommandSyntaxException {
        ...
        if (worldborder.getDamageSafeZone() == pDistance) {
    ...
    private static int setDamageAmount(CommandSourceStack pSource, float pDamagePerBlock) throws CommandSyntaxException {
        WorldBorder worldborder = pSource.getServer().overworld().getWorldBorder();
        if (worldborder.getDamagePerBlock() == pDamagePerBlock) {
    ...
    private static int setCenter(CommandSourceStack pSource, Vec2 pPos) throws CommandSyntaxException {
        WorldBorder worldborder = pSource.getServer().overworld().getWorldBorder();
        if (worldborder.getCenterX() == pPos.x && worldborder.getCenterZ() == pPos.y) {
            throw ERROR_SAME_CENTER.create();
        } else if (!(Math.abs(pPos.x) > 2.9999984E7) && !(Math.abs(pPos.y) > 2.9999984E7)) {
            worldborder.setCenter(pPos.x, pPos.y);
            pSource.sendSuccess(
                () -> Component.translatable(
                    "commands.worldborder.center.success",
                    String.format(Locale.ROOT, "%.2f", pPos.x),
                    String.format(Locale.ROOT, "%.2f", pPos.y)
                ),
                true
            );
            return 0;
    ...
    private static int setSize(CommandSourceStack pSource, double pNewSize, long pTime) throws CommandSyntaxException {
        ...
        if (d0 == pNewSize) {
        ...
        } else if (pNewSize < 1.0) {
            throw ERROR_TOO_SMALL.create();
        } else if (pNewSize > 5.999997E7F) {
            ...
            if (pTime > 0L) {
                ...
                if (pNewSize > d0) {
                    pSource.sendSuccess(
                        () -> Component.translatable(
                            "commands.worldborder.set.grow", String.format(Locale.ROOT, "%.1f", pNewSize), Long.toString(pTime / 1000L)
                        ),
                        true
                    );
                } else {
                    pSource.sendSuccess(
                        () -> Component.translatable(
                            "commands.worldborder.set.shrink", String.format(Locale.ROOT, "%.1f", pNewSize), Long.toString(pTime / 1000L)
                        ),
                        true
                    );
                }
