...
public class SetSpawnCommand {
    ...
    private static int setSpawn(CommandSourceStack pSource, Collection<ServerPlayer> pTargets, BlockPos pPos, float pAngle) {
        ...
        for (ServerPlayer serverplayer : pTargets) {
            serverplayer.setRespawnPosition(new ServerPlayer.RespawnConfig(resourcekey, pPos, pAngle, true), false);
        }
        ...
        if (pTargets.size() == 1) {
            ...
            pSource.sendSuccess(
                () -> Component.translatable(
                    "commands.spawnpoint.success.multiple", pPos.getX(), pPos.getY(), pPos.getZ(), pAngle, s, pTargets.size()
                ),
                true
            );
        }
