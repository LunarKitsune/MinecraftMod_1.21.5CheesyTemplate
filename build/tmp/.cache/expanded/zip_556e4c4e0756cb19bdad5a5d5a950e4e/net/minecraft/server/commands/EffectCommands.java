...
public class EffectCommands {
                    ...
                    Commands.literal("clear")
                        ...
                        .then(
                            Commands.argument("targets", EntityArgument.entities())
                                ...
                                .then(
                                    Commands.argument("effect", ResourceArgument.resource(pContext, Registries.MOB_EFFECT))
                                        .executes(
    ...
    private static int giveEffect(
        ...
        @Nullable Integer pSeconds,
        int pAmplifier,
        boolean pShowParticles
        ...
        if (i == 0) {
            ...
            if (pTargets.size() == 1) {
                pSource.sendSuccess(
                    () -> Component.translatable("commands.effect.give.success.single", mobeffect.getDisplayName(), pTargets.iterator().next().getDisplayName(), j / 20), true
                );
            } else {
                pSource.sendSuccess(() -> Component.translatable("commands.effect.give.success.multiple", mobeffect.getDisplayName(), pTargets.size(), j / 20), true);
            }
    ...
    private static int clearEffect(CommandSourceStack pSource, Collection<? extends Entity> pTargets, Holder<MobEffect> pEffect) throws CommandSyntaxException {
        MobEffect mobeffect = pEffect.value();
        int i = 0;
        ...
        if (i == 0) {
            ...
            if (pTargets.size() == 1) {
                pSource.sendSuccess(
                    () -> Component.translatable("commands.effect.clear.specific.success.single", mobeffect.getDisplayName(), pTargets.iterator().next().getDisplayName()),
                    true
                );
            } else {
                pSource.sendSuccess(() -> Component.translatable("commands.effect.clear.specific.success.multiple", mobeffect.getDisplayName(), pTargets.size()), true);
            }
