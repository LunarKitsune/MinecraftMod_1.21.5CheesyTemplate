...
public class PlaceCommand {
    ...
    public static int placeTemplate(
        ...
        if (optional.isEmpty()) {
            ...
            StructurePlaceSettings structureplacesettings = new StructurePlaceSettings().setMirror(pMirror).setRotation(pRotation).setKnownShape(pStrict);
            if (pIntegrity < 1.0F) {
                structureplacesettings.clearProcessors().addProcessor(new BlockRotProcessor(pIntegrity)).setRandom(StructureBlockEntity.createRandom(pSeed));
            }

            boolean flag = structuretemplate.placeInWorld(
                serverlevel, pPos, pPos, structureplacesettings, StructureBlockEntity.createRandom(pSeed), 2 | (pStrict ? 816 : 0)
            );
            if (!flag) {
                ...
                pSource.sendSuccess(
                    () -> Component.translatable(
                        "commands.place.template.success", Component.translationArg(pTemplate), pPos.getX(), pPos.getY(), pPos.getZ()
                    ),
                    true
                );
                return 1;
