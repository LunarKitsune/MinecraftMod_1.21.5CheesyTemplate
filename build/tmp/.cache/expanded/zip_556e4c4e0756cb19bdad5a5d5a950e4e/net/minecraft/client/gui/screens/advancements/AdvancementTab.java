...
public class AdvancementTab {
    ...
    public void drawContents(GuiGraphics pGuiGraphics, int pX, int pY) {
        ...
        pGuiGraphics.pose().translate(pX, pY, 0.0F);
    ...
    public static AdvancementTab create(Minecraft pMinecraft, AdvancementsScreen pScreen, int pIndex, AdvancementNode pRootNode) {
        ...
        if (optional.isEmpty()) {
            ...
            for (AdvancementTabType advancementtabtype : AdvancementTabType.values()) {
                if ((pIndex % AdvancementTabType.MAX_TABS) < advancementtabtype.getMax()) {
                    return new AdvancementTab(pMinecraft, pScreen, advancementtabtype, pIndex % AdvancementTabType.MAX_TABS, pIndex / AdvancementTabType.MAX_TABS, pRootNode, optional.get());
                }
    ...
    public void scroll(double pDragX, double pDragY) {
        if (this.maxX - this.minX > 234) {
            this.scrollX = Mth.clamp(this.scrollX + pDragX, -(this.maxX - 234), 0.0);
        }

        if (this.maxY - this.minY > 113) {
            this.scrollY = Mth.clamp(this.scrollY + pDragY, -(this.maxY - 113), 0.0);
        }
    }
