...
public class Camera {
    ...
    public void setup(BlockGetter pLevel, Entity pEntity, boolean pDetached, boolean pThirdPersonReverse, float pPartialTick) {
        ...
        if (pEntity.isPassenger()
            ...
            Vec3 vec3 = minecart.getPassengerRidingPosition(pEntity)
                ...
                .add(new Vec3(0.0, Mth.lerp(pPartialTick, this.eyeHeightOld, this.eyeHeight), 0.0));
            ...
            this.setRotation(pEntity.getViewYRot(pPartialTick), pEntity.getViewXRot(pPartialTick));
            this.setPosition(
                Mth.lerp(pPartialTick, pEntity.xo, pEntity.getX()),
                Mth.lerp(pPartialTick, pEntity.yo, pEntity.getY()) + Mth.lerp(pPartialTick, this.eyeHeightOld, this.eyeHeight),
                Mth.lerp(pPartialTick, pEntity.zo, pEntity.getZ())
            );
        }
    ...
    private float getMaxZoom(float pMaxZoom) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            Vec3 vec31 = vec3.add(new Vec3(this.forwards).scale(-pMaxZoom));
    ...
    protected void setRotation(float pYRot, float pXRot) {
        setRotation(pYRot, pXRot, 0.0f);
    }

    public void setRotation(float pYRot, float pXRot, float z) {
        this.xRot = pXRot;
        this.yRot = pYRot;
        this.rotation.rotationYXZ((float) Math.PI - pYRot * (float) (Math.PI / 180.0), -pXRot * (float) (Math.PI / 180.0), z * (float) (Math.PI / 180.0));
    ...
    public static class NearPlane {
        ...
        public Vec3 getPointOnPlane(float pLeftScale, float pUpScale) {
            return this.forward.add(this.up.scale(pUpScale)).subtract(this.left.scale(pLeftScale));
        }
    }
}
