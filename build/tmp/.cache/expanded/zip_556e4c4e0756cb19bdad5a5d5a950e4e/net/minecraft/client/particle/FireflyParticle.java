...
public class FireflyParticle extends TextureSheetParticle {
    ...
    private float getLifetimeProgress(float pAge) {
        return Mth.clamp(pAge / this.lifetime, 0.0F, 1.0F);
    }

    private static float getFadeAmount(float pLifetimeProgress, float pFadeIn, float pFadeOut) {
        if (pLifetimeProgress >= 1.0F - pFadeIn) {
            return (1.0F - pLifetimeProgress) / pFadeIn;
        } else {
            return pLifetimeProgress <= pFadeOut ? pLifetimeProgress / pFadeOut : 1.0F;
        }
    }
    ...
    public static class FireflyProvider implements ParticleProvider<SimpleParticleType> {
        ...
        public FireflyProvider(SpriteSet pSprite) {
            this.sprite = pSprite;
        }
