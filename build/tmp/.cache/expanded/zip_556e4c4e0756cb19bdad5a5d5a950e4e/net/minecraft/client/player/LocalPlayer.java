...
public class LocalPlayer extends AbstractClientPlayer {
    ...
    private void moveTowardsClosestSpace(double pX, double pZ) {
        ...
        if (this.suffocatesAt(blockpos)) {
            double d0 = pX - blockpos.getX();
            double d1 = pZ - blockpos.getZ();
    ...
    private boolean suffocatesAt(BlockPos pPos) {
        ...
        AABB aabb1 = new AABB(
                pPos.getX(), aabb.minY, pPos.getZ(), pPos.getX() + 1.0, aabb.maxY, pPos.getZ() + 1.0
            )
            .deflate(1.0E-7);
    ...
    public void playSound(SoundEvent pSound, float pVolume, float pPitch) {
        ...
        var event = net.minecraftforge.event.ForgeEventFactory.onPlaySoundAtEntity(this.level(), this, holder, this.getSoundSource(), pVolume, pPitch);
    ...
    private Vec2 modifyInput(Vec2 pMoveVector) {
        if (pMoveVector.lengthSquared() == 0.0F) {
            return pMoveVector;
        } else {
            Vec2 vec2 = pMoveVector.scale(0.98F);
    ...
    private static Vec2 modifyInputSpeedForSquareMovement(Vec2 pMoveVector) {
        float f = pMoveVector.length();
        if (f <= 0.0F) {
            return pMoveVector;
        } else {
            Vec2 vec2 = pMoveVector.scale(1.0F / f);
    ...
    private static float distanceToUnitSquare(Vec2 pMoveVector) {
        float f = Math.abs(pMoveVector.x);
        float f1 = Math.abs(pMoveVector.y);
    ...
    private void handlePortalTransitionEffect(boolean pInPortal) {
        ...
        float f = 0.0F;
        if (pInPortal && this.portalProcess != null && this.portalProcess.isInsidePortalThisTick()) {
    ...
    protected void updateAutoJump(float pMovementX, float pMovementZ) {
        if (this.canAutoJump()) {
            ...
            Vec3 vec31 = vec3.add(pMovementX, 0.0, pMovementZ);
            Vec3 vec32 = new Vec3(pMovementX, 0.0, pMovementZ);
    ...
    private boolean vehicleCanSprint(Entity pVehicle) {
        return pVehicle.canSprint() && pVehicle.isLocalInstanceAuthoritative();
    }
