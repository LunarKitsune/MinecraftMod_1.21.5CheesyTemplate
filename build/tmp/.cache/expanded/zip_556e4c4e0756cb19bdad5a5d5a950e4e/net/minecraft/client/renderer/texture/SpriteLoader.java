...
public class SpriteLoader {
    ...
    public SpriteLoader.Preparations stitch(List<SpriteContents> pContents, int pMipLevel, Executor pExecutor) {
        ...
        try (Zone zone = Profiler.get().zone(() -> "stitch " + this.location)) {
            ...
            int k1 = Mth.log2(j1);
            int l1;
            if (k1 < pMipLevel && net.minecraftforge.common.ForgeConfig.CLIENT.allowMipmapLowering()) { // Forge: Do not lower the mipmap level
    ...
    public static CompletableFuture<List<SpriteContents>> runSpriteSuppliers(
        ...
        List<CompletableFuture<SpriteContents>> list = pFactories.stream()
            .map(p_296294_ -> CompletableFuture.supplyAsync(() -> (SpriteContents)p_296294_.apply(pSpriteResourceLoader), pExecutor))
            .toList();
    ...
    public CompletableFuture<SpriteLoader.Preparations> loadAndStitch(
        ...
        return CompletableFuture.<List<Function<SpriteResourceLoader, SpriteContents>>>supplyAsync(
            ...
            .thenCompose(p_296297_ -> runSpriteSuppliers(spriteresourceloader, (List<Function<SpriteResourceLoader, SpriteContents>>)p_296297_, pExecutor))
            .thenApply(p_261393_ -> this.stitch((List<SpriteContents>)p_261393_, pMipLevel, pExecutor));
    }
