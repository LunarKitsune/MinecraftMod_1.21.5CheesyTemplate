...
public final class ModelPart {
    ...
    public void render(PoseStack pPoseStack, VertexConsumer pBuffer, int pPackedLight, int pPackedOverlay, int pColor) {
        if (this.visible) {
            if (!this.cubes.isEmpty() || !this.children.isEmpty()) {
                ...
                if (!this.skipDraw) {
                    this.compile(pPoseStack.last(), pBuffer, pPackedLight, pPackedOverlay, pColor);
                }

                for (ModelPart modelpart : this.children.values()) {
                    modelpart.render(pPoseStack, pBuffer, pPackedLight, pPackedOverlay, pColor);
                }
    ...
    private void visit(PoseStack pPoseStack, ModelPart.Visitor pVisitor, String pPath) {
        if (!this.cubes.isEmpty() || !this.children.isEmpty()) {
            ...
            String s = pPath + "/";
            this.children.forEach((p_171320_, p_171321_) -> p_171321_.visit(pPoseStack, pVisitor, s + p_171320_));
            pPoseStack.popPose();
        }
    }
    ...
    private void compile(PoseStack.Pose pPose, VertexConsumer pBuffer, int pPackedLight, int pPackedOverlay, int pColor) {
        for (ModelPart.Cube modelpart$cube : this.cubes) {
            modelpart$cube.compile(pPose, pBuffer, pPackedLight, pPackedOverlay, pColor);
        }
    }
    ...
    public static class Cube {
        ...
        public void compile(PoseStack.Pose pPose, VertexConsumer pBuffer, int pPackedLight, int pPackedOverlay, int pColor) {
            ...
            for (ModelPart.Polygon modelpart$polygon : this.polygons) {
                Vector3f vector3f1 = pPose.transformNormal(modelpart$polygon.normal, vector3f);
                float f = vector3f1.x();
