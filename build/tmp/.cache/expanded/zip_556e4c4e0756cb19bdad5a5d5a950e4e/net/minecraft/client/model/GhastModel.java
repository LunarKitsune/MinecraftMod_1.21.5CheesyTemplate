...
public class GhastModel extends EntityModel<GhastRenderState> {
    ...
    public static LayerDefinition createBodyLayer() {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            partdefinition.addOrReplaceChild(
                createTentacleName(i), CubeListBuilder.create().texOffs(0, 0).addBox(-1.0F, 0.0F, -1.0F, 2.0F, j, 2.0F), PartPose.offset(f, 24.6F, f1)
            );
        }
    ...
    public void setupAnim(GhastRenderState p_365609_) {
        ...
        for (int i = 0; i < this.tentacles.length; i++) {
            this.tentacles[i].xRot = 0.2F * Mth.sin(p_365609_.ageInTicks * 0.3F + i) + 0.4F;
        }
    }
}