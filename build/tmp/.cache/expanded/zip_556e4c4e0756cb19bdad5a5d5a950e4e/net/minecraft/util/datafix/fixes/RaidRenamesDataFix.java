...
public class RaidRenamesDataFix extends DataFix {
    public RaidRenamesDataFix(Schema pOutputSchema) {
    ...
    private static Dynamic<?> fix(Dynamic<?> pData) {
        return pData.renameAndFixField("Raids", "raids", p_395685_ -> p_395685_.createList(p_395685_.asStream().map(RaidRenamesDataFix::fixRaid)))
            .renameField("Tick", "tick")
            .renameField("NextAvailableID", "next_id");
    }

    private static Dynamic<?> fixRaid(Dynamic<?> pData) {
        return ExtraDataFixUtils.fixInlineBlockPos(pData, "CX", "CY", "CZ", "center")
            .renameField("Id", "id")
