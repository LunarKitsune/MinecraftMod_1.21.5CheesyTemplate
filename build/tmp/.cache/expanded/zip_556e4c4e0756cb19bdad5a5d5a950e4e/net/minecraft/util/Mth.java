...
public class Mth {
    ...
    public static int ceillog2(int pValue) {
        ...
        return MULTIPLY_DE_BRUIJN_BIT_POSITION[(int)(pValue * 125613361L >> 27) & 31];
    }
    ...
    public static long getSeed(int pX, int pY, int pZ) {
        long i = pX * 3129871 ^ pZ * 116129781L ^ pY;
    ...
    public static int lerpInt(float pDelta, int pStart, int pEnd) {
        return pStart + floor(pDelta * (pEnd - pStart));
    }

    public static int lerpDiscrete(float pDelta, int pStart, int pEnd) {
        int i = pEnd - pStart;
        return pStart + floor(pDelta * (i - 1)) + (pDelta > 0.0F ? 1 : 0);
    }
    ...
    public static double wobble(double pInput) {
        return pInput + (2.0 * RandomSource.create(floor(pInput * 3000.0)).nextDouble() - 1.0) * 1.0E-7 / 2.0;
    }
    ...
    public static float length(float pXDistance, float pYDistance) {
        return (float)Math.sqrt(lengthSquared(pXDistance, pYDistance));
    }
    ...
    public static int quantize(double pValue, int pFactor) {
        return floor(pValue / pFactor) * pFactor;
    }
