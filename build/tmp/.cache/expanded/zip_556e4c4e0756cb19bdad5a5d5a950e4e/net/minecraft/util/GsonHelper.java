...
public class GsonHelper {
    ...
    public static boolean isValidNode(@Nullable JsonObject pJson, String pMemberName) {
        return pJson == null ? false : pJson.get(pMemberName) != null;
    }
    ...
    public static Holder<Item> convertToItem(JsonElement pJson, String pMemberName) {
        if (pJson.isJsonPrimitive()) {
            ...
            return BuiltInRegistries.ITEM
                ...
                .orElseThrow(() -> new JsonSyntaxException("Expected " + pMemberName + " to be an item, was unknown string '" + s + "'"));
        } else {
    ...
    @Nullable
    @Contract("_,_,!null->!null;_,_,null->_")
    public static Holder<Item> getAsItem(JsonObject pJson, String pMemberName, @Nullable Holder<Item> pFallback) {
    ...
    public static <T> T getAsObject(JsonObject pJson, String pMemberName, @Nullable T pFallback, JsonDeserializationContext pContext, Class<? extends T> pAdapter) {
        return pJson.has(pMemberName) ? convertToObject(pJson.get(pMemberName), pMemberName, pContext, pAdapter) : pFallback;
    }
    ...
    public static void writeValue(JsonWriter pWriter, @Nullable JsonElement pJsonElement, @Nullable Comparator<String> pSorter) throws IOException {
        if (pJsonElement == null || pJsonElement.isJsonNull()) {
            ...
            for (Entry<String, JsonElement> entry : sortByKeyIfNeeded(pJsonElement.getAsJsonObject().entrySet(), pSorter)) {
    ...
    private static Collection<Entry<String, JsonElement>> sortByKeyIfNeeded(Collection<Entry<String, JsonElement>> pEntries, @Nullable Comparator<String> pSorter) {
        if (pSorter == null) {
            return pEntries;
        } else {
            List<Entry<String, JsonElement>> list = new ArrayList<>(pEntries);
            list.sort(Entry.comparingByKey(pSorter));
            return list;
        }
    }

    public static boolean encodesLongerThan(JsonElement pElement, int pMaxLength) {
        try {
            Streams.write(pElement, new JsonWriter(Streams.writerForAppendable(new GsonHelper.CountedAppendable(pMaxLength))));
            return false;
    ...
    static class CountedAppendable implements Appendable {
        ...
        private Appendable accountChars(int pChars) {
            this.totalCount += pChars;
        ...
        @Override
        public Appendable append(CharSequence pSequence) {
            return this.accountChars(pSequence.length());
        }

        @Override
        public Appendable append(CharSequence pSequence, int pStart, int pEnd) {
            return this.accountChars(pEnd - pStart);
        }

        @Override
        public Appendable append(char pC) {
