...
public class JfrResultJsonSerializer {
    ...
    private JsonElement chunkGen(List<Pair<ChunkStatus, TimedStatSummary<ChunkGenStat>>> pSummary) {
        JsonObject jsonobject = new JsonObject();
        jsonobject.addProperty("durationNanosTotal", pSummary.stream().mapToDouble(p_185567_ -> p_185567_.getSecond().totalDuration().toNanos()).sum());
    ...
    private JsonElement serverTicks(List<TickTimeStat> pStats) {
        if (pStats.isEmpty()) {
            ...
            JsonObject jsonobject = new JsonObject();
            double[] adouble = pStats.stream().mapToDouble(p_185548_ -> p_185548_.currentAverage().toNanos() / 1000000.0).toArray();
