...
public class VanillaHusbandryAdvancements implements AdvancementSubProvider {
    ...
    private static Advancement.Builder addLeashedFrogVariants(
        ...
        sortedVariants(pFrogVariants)
            .forEach(
                p_389707_ -> pBuilder.addCriterion(
                    p_389707_.key().location().toString(),
                    PlayerInteractTrigger.TriggerInstance.itemUsedOnEntity(
                        ItemPredicate.Builder.item().of(pItemRegistry, Items.LEAD),
                        Optional.of(
                            EntityPredicate.wrap(
                                EntityPredicate.Builder.entity()
    ...
    private static <T> Stream<Holder.Reference<T>> sortedVariants(HolderLookup<T> pVariantRegistry) {
        return pVariantRegistry.listElements().sorted(HOLDER_KEY_COMPARATOR);
    }
    ...
    private static Advancement.Builder addCatVariants(Advancement.Builder pBuilder, HolderLookup<CatVariant> pVariantRegistry) {
        sortedVariants(pVariantRegistry)
            .forEach(
                p_389711_ -> pBuilder.addCriterion(
    ...
    private static Advancement.Builder addTamedWolfVariants(Advancement.Builder pBuilder, HolderLookup<WolfVariant> pVariantRegistry) {
        sortedVariants(pVariantRegistry)
            .forEach(
                p_389709_ -> pBuilder.addCriterion(
