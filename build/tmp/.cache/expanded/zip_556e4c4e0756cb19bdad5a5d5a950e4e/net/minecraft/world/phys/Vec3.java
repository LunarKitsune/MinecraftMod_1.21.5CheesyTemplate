...
public class Vec3 implements Position {
    ...
    public static Vec3 fromRGB24(int pPacked) {
        double d0 = (pPacked >> 16 & 0xFF) / 255.0;
        double d1 = (pPacked >> 8 & 0xFF) / 255.0;
        double d2 = (pPacked & 0xFF) / 255.0;
        return new Vec3(d0, d1, d2);
    }

    public static Vec3 atLowerCornerOf(Vec3i pToCopy) {
        return new Vec3(pToCopy.getX(), pToCopy.getY(), pToCopy.getZ());
    }

    public static Vec3 atLowerCornerWithOffset(Vec3i pToCopy, double pOffsetX, double pOffsetY, double pOffsetZ) {
        return new Vec3(pToCopy.getX() + pOffsetX, pToCopy.getY() + pOffsetY, pToCopy.getZ() + pOffsetZ);
    }
    ...
    public Vec3 offsetRandom(RandomSource pRandom, float pFactor) {
        return this.add((pRandom.nextFloat() - 0.5F) * pFactor, (pRandom.nextFloat() - 0.5F) * pFactor, (pRandom.nextFloat() - 0.5F) * pFactor);
    }
    ...
    public Vec3 align(EnumSet<Direction.Axis> pAxes) {
        double d0 = pAxes.contains(Direction.Axis.X) ? Mth.floor(this.x) : this.x;
        double d1 = pAxes.contains(Direction.Axis.Y) ? Mth.floor(this.y) : this.y;
        double d2 = pAxes.contains(Direction.Axis.Z) ? Mth.floor(this.z) : this.z;
        return new Vec3(d0, d1, d2);
    }
    ...
    public Vec3 relative(Direction pDirection, double pLength) {
        ...
        return new Vec3(
            this.x + pLength * vec3i.getX(), this.y + pLength * vec3i.getY(), this.z + pLength * vec3i.getZ()
        );
    }
