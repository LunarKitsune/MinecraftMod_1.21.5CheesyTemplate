...
public abstract class PathNavigation {
    ...
    public Path createPath(BlockPos pPos, int pRegionOffset, int pAccuracy) {
        return this.createPath(ImmutableSet.of(pPos), 8, false, pRegionOffset, pAccuracy);
    }
    ...
    protected void doStuckDetection(Vec3 pPositionVec3) {
        if (this.tick - this.lastStuckCheck > 100) {
            ...
            if (pPositionVec3.distanceToSqr(this.lastStuckCheckPos) < f1 * f1) {
                this.isStuck = true;
                this.stop();
            } else {
                this.isStuck = false;
    ...
    protected static boolean isClearForMovementBetween(Mob pMob, Vec3 pPos1, Vec3 pPos2, boolean pAllowSwimming) {
        Vec3 vec3 = new Vec3(pPos2.x, pPos2.y + pMob.getBbHeight() * 0.5, pPos2.z);
        return pMob.level()
    ...
    public boolean shouldRecomputePath(BlockPos pPos) {
        if (this.hasDelayedRecomputation) {
            ...
            return pPos.closerToCenterThan(vec3, this.path.getNodeCount() - this.path.getNextNodeIndex());
        } else {
            return false;
        }
    }
