...
public class EnderDragon extends Mob implements Enemy {
    ...
    private @Nullable net.minecraft.world.entity.EntityReference<Player> unlimitedLastHurtByPlayer = null;
            ...
            for (int l1 = j; l1 <= i1; l1++) {
                for (int i2 = k; i2 <= j1; i2++) {
                    ...
                    if (!blockstate.isAir() && !blockstate.is(BlockTags.DRAGON_TRANSPARENT)) {
                        if (net.minecraftforge.common.ForgeHooks.canEntityDestroy(pLevel, blockpos, this) && !blockstate.is(BlockTags.DRAGON_IMMUNE)) {
    ...
    protected void tickDeath() {
        ...
        if (this.level() instanceof ServerLevel serverlevel) {
            if (this.dragonDeathTime > 150 && this.dragonDeathTime % 5 == 0 && serverlevel.getGameRules().getBoolean(GameRules.RULE_DOMOBLOOT)) {
                int award = net.minecraftforge.event.ForgeEventFactory.getExperienceDrop(this, this.getUnlimitedLastHurtByPlayer(), Mth.floor((float)i * 0.08F));
        ...
        if (this.dragonDeathTime == 200 && this.level() instanceof ServerLevel serverlevel1) {
            if (serverlevel1.getGameRules().getBoolean(GameRules.RULE_DOMOBLOOT)) {
                int award = net.minecraftforge.event.ForgeEventFactory.getExperienceDrop(this, this.getUnlimitedLastHurtByPlayer(), Mth.floor((float)i * 0.2F));
    ...
    public void readAdditionalSaveData(CompoundTag pCompound) {
        ...
        pCompound.getInt("DragonPhase").ifPresent(p_395552_ -> this.phaseManager.setPhase(EnderDragonPhase.getById(p_395552_)));
        this.dragonDeathTime = pCompound.getIntOr("DragonDeathTime", 0);
    }
    ...
    public void onCrystalDestroyed(ServerLevel pLevel, EndCrystal pCrystal, BlockPos pPos, DamageSource pDamageSource) {
        Player player;
        if (pDamageSource.getEntity() instanceof Player player1) {
            player = player1;
        } else {
            player = pLevel.getNearestPlayer(CRYSTAL_DESTROY_TARGETING, pPos.getX(), pPos.getY(), pPos.getZ());
        }
