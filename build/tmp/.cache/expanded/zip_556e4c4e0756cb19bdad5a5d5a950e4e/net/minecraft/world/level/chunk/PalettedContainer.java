...
public class PalettedContainer<T> implements PaletteResize<T>, PalettedContainerRO<T> {
    ...
    private static <T, C extends PalettedContainerRO<T>> Codec<C> codec(
        ...
        return RecordCodecBuilder.<PalettedContainerRO.PackedData>create(
                p_327417_ -> p_327417_.group(
                        pCodec.mapResult(ExtraCodecs.orElsePartial(pValue)).listOf().fieldOf("palette").forGetter(PalettedContainerRO.PackedData::paletteEntries),
            ...
            .comapFlatMap(
                p_238262_ -> pUnpacker.read(pRegistry, pStrategy, (PalettedContainerRO.PackedData<T>)p_238262_),
                p_238263_ -> p_238263_.pack(pRegistry, pStrategy)
            );
    }
    ...
    public void getAll(Consumer<T> pConsumer) {
        ...
        intset.forEach(p_238274_ -> pConsumer.accept(palette.valueFor(p_238274_)));
    }
    ...
    public void read(FriendlyByteBuf pBuffer) {
        ...
        try {
            ...
            pBuffer.readFixedSizeLongArray(data.storage.getRaw());
            this.data = data;
        } finally {
            this.release();
        }
        ...
        if (j == 0) {
            ...
            try {
                if (configuration.factory() == PalettedContainer.Strategy.GLOBAL_PALETTE_FACTORY) {
                    Palette<T> palette = new HashMapPalette<>(pRegistry, j, (p_238278_, p_238279_) -> 0, list);
                    SimpleBitStorage simplebitstorage = new SimpleBitStorage(j, i, along);
                    int[] aint = new int[i];
                    simplebitstorage.unpack(aint);
                    swapPalette(aint, p_238283_ -> pRegistry.getId(palette.valueFor(p_238283_)));
    ...
    public void count(PalettedContainer.CountConsumer<T> pCountConsumer) {
        if (this.data.palette.getSize() == 1) {
            ...
            int2intopenhashmap.int2IntEntrySet()
                .forEach(p_238271_ -> pCountConsumer.accept(this.data.palette.valueFor(p_238271_.getIntKey()), p_238271_.getIntValue()));
        }
    }
    ...
    record Data<T>(PalettedContainer.Configuration<T> configuration, BitStorage storage, Palette<T> palette) {
        ...
        public void write(FriendlyByteBuf pBuffer) {
            ...
            pBuffer.writeFixedSizeLongArray(this.storage.getRaw());
        }
