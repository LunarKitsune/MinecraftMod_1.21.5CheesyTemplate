...
public record BlocksAttacks(
    ...
    public void disable(ServerLevel pLevel, LivingEntity pEntity, float pDuration, ItemStack pStack) {
        int i = this.disableBlockingForTicks(pDuration);
        if (i > 0) {
            if (pEntity instanceof Player player) {
                player.getCooldowns().addCooldown(pStack, i);
            }

            pEntity.stopUsingItem();
            this.disableSound
                .ifPresent(
                    p_395865_ -> pLevel.playSound(
                        null,
                        pEntity.getX(),
                        pEntity.getY(),
                        pEntity.getZ(),
    ...
    public void hurtBlockingItem(Level pLevel, ItemStack pStack, LivingEntity pEntity, InteractionHand pHand, float pDamage) {
        if (pEntity instanceof Player player) {
            ...
            int i = this.itemDamage.apply(pDamage);
            if (i > 0) {
                pStack.hurtAndBreak(i, pEntity, LivingEntity.getSlotForHand(pHand));
            }
        }
    }

    private int disableBlockingForTicks(float pDuration) {
        float f = pDuration * this.disableCooldownScale;
    ...
    public float resolveBlockedDamage(DamageSource pDamageSource, float pDamageAmount, double pHorizontalAngle) {
        ...
        for (BlocksAttacks.DamageReduction blocksattacks$damagereduction : this.damageReductions) {
            f += blocksattacks$damagereduction.resolve(pDamageSource, pDamageAmount, pHorizontalAngle);
        }

        return Mth.clamp(f, 0.0F, pDamageAmount);
    }
    ...
    public record DamageReduction(float horizontalBlockingAngle, Optional<HolderSet<DamageType>> type, float base, float factor) {
        ...
        public float resolve(DamageSource pDamageSource, float pDamageAmount, double pHorizontalAngle) {
            if (pHorizontalAngle > (float) (Math.PI / 180.0) * this.horizontalBlockingAngle) {
                return 0.0F;
            } else {
                return this.type.isPresent() && !this.type.get().contains(pDamageSource.typeHolder())
                    ? 0.0F
                    : Mth.clamp(this.base + this.factor * pDamageAmount, 0.0F, pDamageAmount);
            }
        }
    }
    ...
    public record ItemDamageFunction(float threshold, float base, float factor) {
        ...
        public int apply(float pDamageAmount) {
            return pDamageAmount < this.threshold ? 0 : Mth.floor(this.base + this.factor * pDamageAmount);
        }
    }
}