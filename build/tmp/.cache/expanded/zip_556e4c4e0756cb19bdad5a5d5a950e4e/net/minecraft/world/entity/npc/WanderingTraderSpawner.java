...
public class WanderingTraderSpawner implements CustomSpawner {
    ...
    private boolean spawn(ServerLevel pServerLevel) {
        ...
        if (player == null) {
            ...
            if (blockpos2 != null && this.hasEnoughSpace(pServerLevel, blockpos2)) {
                ...
                WanderingTrader wanderingtrader = EntityType.WANDERING_TRADER.spawn(pServerLevel, blockpos2, EntitySpawnReason.EVENT);
    ...
    private void tryToSpawnLlamaFor(ServerLevel pServerLevel, WanderingTrader pTrader, int pMaxDistance) {
        ...
        if (blockpos != null) {
            TraderLlama traderllama = EntityType.TRADER_LLAMA.spawn(pServerLevel, blockpos, EntitySpawnReason.EVENT);
            if (traderllama != null) {
    ...
    private BlockPos findSpawnPositionNear(LevelReader pLevel, BlockPos pPos, int pMaxDistance) {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (spawnplacementtype.isSpawnPositionOk(pLevel, blockpos1, EntityType.WANDERING_TRADER)) {
                blockpos = blockpos1;
                break;
            }
        }
