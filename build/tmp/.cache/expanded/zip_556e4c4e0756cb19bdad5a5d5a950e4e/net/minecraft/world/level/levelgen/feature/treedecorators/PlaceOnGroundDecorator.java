...
public class PlaceOnGroundDecorator extends TreeDecorator {
    ...
    public PlaceOnGroundDecorator(int pTries, int pRadius, int pHeight, BlockStateProvider pBlockStateProvider) {
        this.tries = pTries;
        this.radius = pRadius;
        this.height = pHeight;
        this.blockStateProvider = pBlockStateProvider;
    }
    ...
    private void attemptToPlaceBlockAbove(TreeDecorator.Context pContext, BlockPos pPos) {
        BlockPos blockpos = pPos.above();
        if (pContext.level().isStateAtPosition(blockpos, p_395132_ -> p_395132_.isAir() || p_395132_.is(Blocks.VINE))
            && pContext.checkBlock(pPos, BlockBehaviour.BlockStateBase::isSolidRender)) {
            pContext.setBlock(blockpos, this.blockStateProvider.getState(pContext.random(), blockpos));
        }
    }
}