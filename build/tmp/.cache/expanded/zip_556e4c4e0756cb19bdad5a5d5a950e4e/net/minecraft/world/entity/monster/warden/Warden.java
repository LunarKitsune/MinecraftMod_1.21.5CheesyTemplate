...
public class Warden extends Monster implements VibrationSystem {
    ...
    public float getTendrilAnimation(float pPartialTick) {
        return Mth.lerp(pPartialTick, this.tendrilAnimationO, this.tendrilAnimation) / 10.0F;
    }

    public float getHeartAnimation(float pPartialTick) {
        return Mth.lerp(pPartialTick, this.heartAnimationO, this.heartAnimation) / 10.0F;
    }

    private void clientDiggingParticles(AnimationState pAnimationState) {
        if ((float)pAnimationState.getTimeInMillis(this.tickCount) < 4500.0F) {
    ...
    public void addAdditionalSaveData(CompoundTag p_219434_) {
        ...
        p_219434_.store("anger", AngerManagement.codec(this::canTargetEntity), registryops, this.angerManagement);
        p_219434_.store("listener", VibrationSystem.Data.CODEC, registryops, this.vibrationData);
    }
    ...
    public void readAdditionalSaveData(CompoundTag p_219415_) {
        ...
        RegistryOps<Tag> registryops = this.registryAccess().createSerializationContext(NbtOps.INSTANCE);
        this.angerManagement = p_219415_.read("anger", AngerManagement.codec(this::canTargetEntity), registryops)
            .orElseGet(() -> new AngerManagement(this::canTargetEntity, Collections.emptyList()));
        this.syncClientAngerLevel();
        this.vibrationData = p_219415_.read("listener", VibrationSystem.Data.CODEC, registryops).orElseGet(VibrationSystem.Data::new);
    }
