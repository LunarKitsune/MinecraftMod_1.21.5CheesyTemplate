...
public class WolfSoundVariants {
    ...
    private static ResourceKey<WolfSoundVariant> createKey(WolfSoundVariants.SoundSet pSoundSet) {
        return ResourceKey.create(Registries.WOLF_SOUND_VARIANT, ResourceLocation.withDefaultNamespace(pSoundSet.getIdentifier()));
    }

    public static void bootstrap(BootstrapContext<WolfSoundVariant> pContext) {
        register(pContext, CLASSIC, WolfSoundVariants.SoundSet.CLASSIC);
        register(pContext, PUGLIN, WolfSoundVariants.SoundSet.PUGLIN);
        register(pContext, SAD, WolfSoundVariants.SoundSet.SAD);
        register(pContext, ANGRY, WolfSoundVariants.SoundSet.ANGRY);
        register(pContext, GRUMPY, WolfSoundVariants.SoundSet.GRUMPY);
        register(pContext, BIG, WolfSoundVariants.SoundSet.BIG);
        register(pContext, CUTE, WolfSoundVariants.SoundSet.CUTE);
    }
    ...
    private static void register(BootstrapContext<WolfSoundVariant> pContext, ResourceKey<WolfSoundVariant> pKey, WolfSoundVariants.SoundSet pSoundSet) {
        pContext.register(pKey, SoundEvents.WOLF_SOUNDS.get(pSoundSet));
    }

    public static Holder<WolfSoundVariant> pickRandomSoundVariant(RegistryAccess pRegistryAccess, RandomSource pRandom) {
        return pRegistryAccess.lookupOrThrow(Registries.WOLF_SOUND_VARIANT).getRandom(pRandom).orElseThrow();
    }
    ...
    public static enum SoundSet {
        ...
        private SoundSet(final String pIndentifier, final String pSoundEventSuffix) {
            this.identifier = pIndentifier;
            this.soundEventSuffix = pSoundEventSuffix;
        }
