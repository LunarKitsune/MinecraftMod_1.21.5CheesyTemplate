...
public final class Biome {
    ...
    private float getHeightAdjustedTemperature(BlockPos pPos, int pSeaLevel) {
        ...
        int i = pSeaLevel + 17;
        if (pPos.getY() > i) {
            float f1 = (float)(TEMPERATURE_NOISE.getValue(pPos.getX() / 8.0F, pPos.getZ() / 8.0F, false) * 8.0);
            return f - (f1 + pPos.getY() - i) * 0.05F / 40.0F;
        } else {
            return f;
        }
    }
