...
public abstract class ProjectileWeaponItem extends Item {
    ...
    protected void shoot(
        ...
        float f1 = pProjectileItems.size() == 1 ? 0.0F : 2.0F * f / (pProjectileItems.size() - 1);
        float f2 = (pProjectileItems.size() - 1) % 2 * f1 / 2.0F;
        float f3 = 1.0F;
    ...
    protected static List<ItemStack> draw(ItemStack pWeapon, ItemStack pAmmo, LivingEntity pShooter) {
        if (pAmmo.isEmpty()) {
            ...
            for (int j = 0; j < i; j++) {
                ItemStack itemstack = useAmmo(pWeapon, j == 0 ? pAmmo : itemstack1, pShooter, j > 0 || infinite);
                if (!itemstack.isEmpty()) {
                    list.add(itemstack);
                }
            }
