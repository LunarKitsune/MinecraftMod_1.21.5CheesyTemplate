...
public class RandomStroll {
    ...
    public static OneShot<PathfinderMob> stroll(float pSpeedModifier, boolean pMayStrollFromWater) {
        return strollFlyOrSwim(pSpeedModifier, p_258601_ -> LandRandomPos.getPos(p_258601_, 10, 7), pMayStrollFromWater ? p_258615_ -> true : p_390571_ -> !p_390571_.isInWater());
    }
    ...
    public static BehaviorControl<PathfinderMob> swim(float pSpeedModifier) {
        return strollFlyOrSwim(pSpeedModifier, RandomStroll::getTargetSwimPos, Entity::isInWater);
    }
    ...
    private static Vec3 getTargetSwimPos(PathfinderMob pMob) {
        ...
        for (int[] aint : SWIM_XY_DISTANCE_TIERS) {
            if (vec3 == null) {
                ...
                vec31 = pMob.position().add(pMob.position().vectorTo(vec3).normalize().multiply(aint[0], aint[1], aint[0]));
            }
