...
public class Axolotl extends Animal implements Bucketable {
    ...
    public void addAdditionalSaveData(CompoundTag p_149158_) {
        ...
        p_149158_.store("Variant", Axolotl.Variant.LEGACY_CODEC, this.getVariant());
    ...
    public void readAdditionalSaveData(CompoundTag p_149145_) {
        ...
        this.setVariant(p_149145_.read("Variant", Axolotl.Variant.LEGACY_CODEC).orElse(Axolotl.Variant.DEFAULT));
        this.setFromBucket(p_149145_.getBooleanOr("FromBucket", false));
    }
    ...
    protected void handleAirSupply(ServerLevel pLevel, int pAirSupply) {
        if (this.isAlive() && !this.isInWaterOrRain()) {
            this.setAirSupply(pAirSupply - 1);
            if (this.getAirSupply() == -20) {
                this.setAirSupply(0);
                this.hurtServer(pLevel, this.damageSources().dryOut(), 2.0F);
            }
        } else {
    ...
    public void saveToBucketTag(ItemStack p_149187_) {
        ...
        CustomData.update(DataComponents.BUCKET_ENTITY_DATA, p_149187_, p_390659_ -> {
            p_390659_.putInt("Age", this.getAge());
            Brain<?> brain = this.getBrain();
            if (brain.hasMemoryValue(MemoryModuleType.HAS_HUNTING_COOLDOWN)) {
                p_390659_.putLong("HuntingCooldown", brain.getTimeUntilExpiry(MemoryModuleType.HAS_HUNTING_COOLDOWN));
            }
        });
    }
    ...
    public void applySupportingEffects(Player pPlayer) {
        ...
        pPlayer.removeEffect(MobEffects.MINING_FATIGUE);
    }
