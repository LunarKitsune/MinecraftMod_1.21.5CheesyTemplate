...
public class VillageSiege implements CustomSpawner {
                ...
                if (pLevel.isVillage(blockpos) && !pLevel.getBiome(blockpos).is(BiomeTags.WITHOUT_ZOMBIE_SIEGES)) {
                    for (int i = 0; i < 10; i++) {
                        ...
                        if (siegeLocation != null) {
                            if (net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(new net.minecraftforge.event.village.VillageSiegeEvent(this, pLevel, player, siegeLocation)))
                                return false;
    ...
    private void trySpawn(ServerLevel pLevel) {
        ...
        if (vec3 != null) {
            Zombie zombie;
            try {
                zombie = EntityType.ZOMBIE.create(pLevel, EntitySpawnReason.EVENT); //Forge: Direct Initialization is deprecated, use EntityType.
                zombie.finalizeSpawn(pLevel, pLevel.getCurrentDifficultyAt(zombie.blockPosition()), EntitySpawnReason.EVENT, null);
            } catch (Exception exception) {
                LOGGER.warn("Failed to create zombie for village siege at {}", vec3, exception);
                return;
            }

            zombie.snapTo(vec3.x, vec3.y, vec3.z, pLevel.random.nextFloat() * 360.0F, 0.0F);
    ...
    private Vec3 findRandomSpawnPos(ServerLevel pLevel, BlockPos pPos) {
        for (int i = 0; i < 10; i++) {
            ...
            if (pLevel.isVillage(blockpos) && Monster.checkMonsterSpawnRules(EntityType.ZOMBIE, pLevel, EntitySpawnReason.EVENT, blockpos, pLevel.random)) {
