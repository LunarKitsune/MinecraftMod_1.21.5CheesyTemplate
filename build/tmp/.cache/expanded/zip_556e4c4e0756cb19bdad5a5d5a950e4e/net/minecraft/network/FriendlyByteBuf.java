...
public class FriendlyByteBuf extends ByteBuf {
    ...
    public FriendlyByteBuf writeLongArray(long[] pArray) {
        writeLongArray(this, pArray);
        return this;
    }

    public static void writeLongArray(ByteBuf pBuffer, long[] pArray) {
        VarInt.write(pBuffer, pArray.length);
        writeFixedSizeLongArray(pBuffer, pArray);
    }

    public FriendlyByteBuf writeFixedSizeLongArray(long[] pArray) {
        writeFixedSizeLongArray(this, pArray);
        return this;
    }

    public static void writeFixedSizeLongArray(ByteBuf pBuffer, long[] pArray) {
        for (long i : pArray) {
            pBuffer.writeLong(i);
        }
    }
    ...
    public long[] readFixedSizeLongArray(long[] pOutput) {
        return readFixedSizeLongArray(this, pOutput);
    }

    public static long[] readLongArray(ByteBuf pBuffer) {
        int i = VarInt.read(pBuffer);
        int j = pBuffer.readableBytes() / 8;
        if (i > j) {
            throw new DecoderException("LongArray with size " + i + " is bigger than allowed " + j);
        } else {
            return readFixedSizeLongArray(pBuffer, new long[i]);
        }
    }

    public static long[] readFixedSizeLongArray(ByteBuf pBuffer, long[] pOutput) {
        for (int i = 0; i < pOutput.length; i++) {
            pOutput[i] = pBuffer.readLong();
        }
