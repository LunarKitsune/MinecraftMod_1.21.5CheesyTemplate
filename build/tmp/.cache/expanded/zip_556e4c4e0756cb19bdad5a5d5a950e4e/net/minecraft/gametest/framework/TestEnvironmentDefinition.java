...
public interface TestEnvironmentDefinition {
    ...
    static MapCodec<? extends TestEnvironmentDefinition> bootstrap(Registry<MapCodec<? extends TestEnvironmentDefinition>> pRegistry) {
        Registry.register(pRegistry, "all_of", TestEnvironmentDefinition.AllOf.CODEC);
        Registry.register(pRegistry, "game_rules", TestEnvironmentDefinition.SetGameRules.CODEC);
        Registry.register(pRegistry, "time_of_day", TestEnvironmentDefinition.TimeOfDay.CODEC);
        Registry.register(pRegistry, "weather", TestEnvironmentDefinition.Weather.CODEC);
        return Registry.register(pRegistry, "function", TestEnvironmentDefinition.Functions.CODEC);
    }

    void setup(ServerLevel pLevel);

    default void teardown(ServerLevel pLevel) {
    }
    ...
    public record AllOf(List<Holder<TestEnvironmentDefinition>> definitions) implements TestEnvironmentDefinition {
        ...
        public AllOf(TestEnvironmentDefinition... pDefinitons) {
            this(Arrays.stream(pDefinitons).map(Holder::direct).toList());
        }
    ...
    public record Functions(Optional<ResourceLocation> setupFunction, Optional<ResourceLocation> teardownFunction) implements TestEnvironmentDefinition {
        ...
        private static void run(ServerLevel pLevel, ResourceLocation pFunction) {
            MinecraftServer minecraftserver = pLevel.getServer();
            ServerFunctionManager serverfunctionmanager = minecraftserver.getFunctions();
            Optional<CommandFunction<CommandSourceStack>> optional = serverfunctionmanager.get(pFunction);
            if (optional.isPresent()) {
                CommandSourceStack commandsourcestack = minecraftserver.createCommandSourceStack().withPermission(2).withSuppressedOutput().withLevel(pLevel);
                serverfunctionmanager.execute(optional.get(), commandsourcestack);
            } else {
                LOGGER.error("Test Batch failed for non-existent function {}", pFunction);
            }
        }
    ...
    public record SetGameRules(
        ...
        public static <S, T extends GameRules.Value<T>> TestEnvironmentDefinition.SetGameRules.Entry<S, T> entry(GameRules.Key<T> pKey, S pValue) {
            return new TestEnvironmentDefinition.SetGameRules.Entry<>(pKey, pValue);
        }
        ...
        public record Entry<S, T extends GameRules.Value<T>>(GameRules.Key<T> key, S value) {
            public static <S, T extends GameRules.Value<T>> Codec<TestEnvironmentDefinition.SetGameRules.Entry<S, T>> codec(
                Class<T> pValueClass, Codec<S> pValueCodec
            ) {
                return RecordCodecBuilder.create(
                    p_396688_ -> p_396688_.group(
                            GameRules.keyCodec(pValueClass).fieldOf("rule").forGetter(TestEnvironmentDefinition.SetGameRules.Entry::key),
                            pValueCodec.fieldOf("value").forGetter(TestEnvironmentDefinition.SetGameRules.Entry::value)
                        )
    ...
    public record Weather(TestEnvironmentDefinition.Weather.Type weather) implements TestEnvironmentDefinition {
        ...
        public static enum Type implements StringRepresentable {
            ...
            private Type(final String pId, final int pClearTime, final int pRainTime, final boolean pRaining, final boolean pThundering) {
                this.id = pId;
                this.clearTime = pClearTime;
                this.rainTime = pRainTime;
                this.raining = pRaining;
                this.thundering = pThundering;
            }

            void apply(ServerLevel pLevel) {
                pLevel.setWeatherParameters(this.clearTime, this.rainTime, this.raining, this.thundering);
            }
