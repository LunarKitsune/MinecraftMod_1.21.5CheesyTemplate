...
public class GameTestRunner {
    ...
    void runBatch(final int pIndex) {
        if (pIndex >= this.batches.size()) {
            ...
            final GameTestBatch gametestbatch = this.batches.get(pIndex);
            ...
            this.batchListeners.forEach(p_389766_ -> p_389766_.testBatchStarting(gametestbatch));
            ...
            multipletesttracker.addListener(new GameTestListener() {
                private void testCompleted() {
                    if (multipletesttracker.isDone()) {
                        GameTestRunner.this.batchListeners.forEach(p_389769_ -> p_389769_.testBatchFinished(gametestbatch));
    ...
    private void runScheduledRerunTests() {
        if (!this.scheduledForRerun.isEmpty()) {
            LOGGER.info(
                "Starting re-run of tests: {}", this.scheduledForRerun.stream().map(p_389767_ -> p_389767_.id().toString()).collect(Collectors.joining(", "))
            );
    ...
    private Optional<GameTestInfo> spawn(GameTestInfo pTest) {
        return pTest.getTestBlockPos() == null ? this.newStructureSpawner.spawnStructure(pTest) : this.existingStructureSpawner.spawnStructure(pTest);
    }
    ...
    public interface StructureSpawner {
        GameTestRunner.StructureSpawner IN_PLACE = p_389770_ -> Optional.of(p_389770_.prepareTestStructure().startExecution(1));
